...
class OllamaMessageSerializer:
	"""
	Sérialiseur pour convertir une liste de messages en prompt texte pour Ollama.
	"""
	@staticmethod
	def serialize_messages(messages: list[BaseMessage]) -> str:
		...
		for message in messages:
			if isinstance(message, SystemMessage):
				# On peut ignorer ou préfixer, selon le besoin
				if isinstance(message.content, str):
					prompt += f"[System]\n{message.content}\n"
				...
				if isinstance(message.content, str):
					...
					# Si content est une liste de ContentPartTextParam
					for part in message.content:
						if hasattr(part, 'text'):
							prompt += f"User: {part.text}\n"
			elif isinstance(message, AssistantMessage):
				if isinstance(message.content, str):
